...
function SOTA_OnTimer(self, elapsed)
...
function SOTA_OnChatMsgAddon(event, ...)
	...
	if (prefix == SOTA_MESSAGE_PREFIX) then	
...
--]]
function SOTA_HandleGuildChatMessage(event, ...)
...
--]]
function SOTA_HandleRaidChatMessage(event, ...)
	local message, sender = ...;
...
--	Since 0.1.0
--]]
function SOTA_OnChatWhisper(event, ...)	
	local message, sender = ...;
...
function SOTA_OnEvent(event, ...)
	...
	elseif (event == "CHAT_MSG_GUILD") then
		SOTA_HandleGuildChatMessage(event, ...);
	elseif (event == "CHAT_MSG_RAID" or event == "CHAT_MSG_RAID_LEADER") then
		SOTA_HandleRaidChatMessage(event, ...);
...
function SOTA_OnLoad()
	...
	C_ChatInfo.RegisterAddonMessagePrefix(SOTA_MESSAGE_PREFIX);

	SOTAEventFrame:RegisterEvent("ADDON_LOADED");
	SOTAEventFrame:RegisterEvent("GUILD_ROSTER_UPDATE");
	SOTAEventFrame:RegisterEvent("RAID_ROSTER_UPDATE");
	SOTAEventFrame:RegisterEvent("CHAT_MSG_GUILD");
	SOTAEventFrame:RegisterEvent("CHAT_MSG_RAID");
